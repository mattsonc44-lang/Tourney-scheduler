...
function generateSchedule({groups,teams,courts,gameDurationMins,linkedGroups,courtGroupPrimary,pinnedMatchups,excludedMatchups}){
  ...
  for(const pg of pinnedGames){
    ...
    // Force-place (override back-to-back for pinned games — coordinator's choice)
                  ...
                  <Card key={court.id}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
                      <div>
                        ...
                        {(court.windows||[]).length>0&&(()=>{const tot=(court.windows||[]).reduce((s,w)=>{const d=timeMins(w.close||"00:00")-timeMins(w.open||"00:00");return s+(d>0?d:0);},0);const sl=Math.floor(tot/gameDuration);const dates=[...new Set((court.windows||[]).map(w=>w.date).filter(Boolean))];return <div style={{marginTop:8,fontSize:11,color:P.muted}}>{dates.length} day{dates.length!==1?"s":""} · {tot} min · ~{sl} slot{sl!==1?"s":""}</div>;})()}
                      </div>
                      <div>
          ...
          <div>
            ...
            <Card style={{maxWidth:420}}>
              ...
              <div style={{background:P.bg,borderRadius:8,padding:14,border:`1px solid ${P.border}`}}>
                <Lbl>Summary</Lbl>
                {[["Total games",totalGames],["Pinned",pinnedCount],["Auto-schedule",totalGames-pinnedCount],["Courts",courts.length],["Available slots",totalSlots],["Capacity",totalSlots>=totalGames?"✅ Enough":"⚠️ May not fit"]].map(([l,v])=>(
