...
// ─── STORAGE HELPERS ─────────────────────────────────────────────────────────
const LS_INDEX = "ts_index";        // list of { id, name, savedAt }
...
// Serialize tournament state → plain JSON-safe object
function serializeState(state) {
  const matchupsPlain = {};
  for (const [gid, s] of Object.entries(state.groupMatchups||{})) {
    matchupsPlain[gid] = s instanceof Set ? [...s] : (Array.isArray(s) ? s : []);
  }
  return {
    groups: state.groups,
    teams: state.teams,
    ...
    groupMatchups: matchupsPlain,
  };
}

// Deserialize → restore Sets for groupMatchups
function deserializeState(raw) {
  const matchups = {};
  for (const [gid, arr] of Object.entries(raw.groupMatchups||{})) {
    matchups[gid] = new Set(Array.isArray(arr) ? arr : []);
  }
  return { ...raw, groupMatchups: matchups };
}
...
function saveTournament(id, name, state) {
  ...
  const idx = loadIndex();
  const existing = idx.findIndex(x=>x.id===id);
...
function loadTournament(id) {
  try {
    const raw = JSON.parse(localStorage.getItem(lsKey(id))||"null");
    if (!raw) return null;
...
function blankState() {
  ...
  return {
    ...
    teams: {},
    courts: [
      { id:"c1", name:"Court 1", location:"", windows:[{id:"w1",date:today,open:"08:00",close:"17:00"}] },
    ],
    courtGroupPrimary: {},
    gameDuration: 30,
    linkedGroups: [],
  ...
  return (
    <div onClick={onToggle} style={{
      ...
      <div style={{width:14,height:14,borderRadius:3,flexShrink:0,
        ...
        fontSize:9,color:"#0d1b2a",fontWeight:900}}>{enabled?"✓":""}</div>
      <span style={{fontWeight:700,fontSize:12,color:enabled?teamA.color:P.muted}}>{teamA.name}</span>
      <span style={{color:P.border,fontSize:11}}>vs</span>
      <span style={{fontWeight:700,fontSize:12,color:enabled?teamB.color:P.muted}}>{teamB.name}</span>
    </div>
  );
}
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{padding:"18px 20px",borderBottom:`1px solid ${P.border}`,display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{fontWeight:800,fontSize:16,color:P.accent,flex:1}}>Tournaments</div>
          <span onClick={onClose} style={{cursor:"pointer",color:P.muted,fontSize:18,lineHeight:1}}>✕</span>
        </div>

        {/* Current tournament save */}
        <div style={{padding:"14px 20px",borderBottom:`1px solid ${P.border}`,background:P.bg}}>
          <div style={{color:P.muted,fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>Current Tournament</div>
        ...
        <div style={{padding:"14px 20px",borderBottom:`1px solid ${P.border}`}}>
          <div style={{color:P.muted,fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>New Tournament</div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"14px 20px"}}>
          <div style={{color:P.muted,fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:12}}>
            Saved ({index.length})
          </div>
          {index.length===0&&(
            <div style={{color:P.muted,fontSize:13,fontStyle:"italic",textAlign:"center",padding:"20px 0"}}>
            ...
            {[...index].sort((a,b)=>b.savedAt.localeCompare(a.savedAt)).map(entry=>{
              ...
              return (
                <div key={entry.id} style={{
                  background:isCurrent?P.accent+"18":P.bg,
                  border:`1px solid ${isCurrent?P.accent+"66":P.border}`,
                  borderRadius:8,padding:"12px 14px",
                }}>
                  {isRenaming ? (
                    <div style={{display:"flex",gap:6}}>
                      <Inp value={renameVal} onChange={setRenameVal} placeholder="New name…"
                        style={{fontSize:12}}/>
                      ...
                      <div style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:6}}>
                        <div style={{flex:1}}>
                          <div style={{fontWeight:700,fontSize:13,color:isCurrent?P.accent:P.text}}>
                            {entry.name}
                            {isCurrent&&<span style={{fontSize:10,color:P.accent,marginLeft:6,fontWeight:400}}>● current</span>}
                          </div>
...
function generateSchedule({ groups, teams, courts, gameDurationMins, linkedGroups, courtGroupPrimary, groupMatchups }) {
  ...
  for (const group of groups) {
    const enabledKeys = groupMatchups[group.id];
  ...
  const resultSlots=[];const usedCourtSlot={};const slotTeamsPlaying={};
  ...
  for(const slotKey of allSlotKeys){
    ...
    for(const court of availCourts){
      ...
      const idx=sorted.findIndex(m=>{
        const{home,away}=m;const playing=slotTeamsPlaying[slotKey]||new Set();
        if(playing.has(home)||playing.has(away))return false;
        const prevKey=prevSlotKey[slotKey];
        if(prevKey!==undefined){const pp=slotTeamsPlaying[prevKey]||new Set();if(pp.has(home)||pp.has(away))return false;}
      ...
      resultSlots.push({slotKey,dayIdx:meta.dayIdx,date:meta.date,absTimeMins:meta.absTimeMins,
        timeLabel:meta.timeLabel,courtId:court.id,match,
      ...
      slotTeamsPlaying[slotKey].add(match.home);slotTeamsPlaying[slotKey].add(match.away);
    }
  }
  const warnings=[];
  if(unscheduled.length>0)warnings.push(`${unscheduled.length} game(s) could not be scheduled — check court availability or constraints.`);
...
export default function App() {
  ...
  // Tournament identity
  const [tournamentId,   setTournamentId]   = useState(null);  // null = unsaved new
  const [tournamentName, setTournamentName] = useState("");
  const [savedFlash,     setSavedFlash]     = useState(false);

  // ── Tournament state (everything that gets saved) ──────────────────────
  const [groups,            setGroups]            = useState([]);
  const [teams,             setTeams]             = useState({});
  const [groupMatchups,     setGroupMatchups]     = useState({});
  const [courts,            setCourts]            = useState([
    { id:"c1", name:"Court 1", location:"", windows:[{id:"w1",date:todayStr(),open:"08:00",close:"17:00"}] },
  ]);
  ...
  const currentState = useCallback(()=>({
    groups, teams, courts, courtGroupPrimary, gameDuration, linkedGroups, groupMatchups,
  }),[groups,teams,courts,courtGroupPrimary,gameDuration,linkedGroups,groupMatchups]);
  ...
  const applyState = (st) => {
    ...
    setTeams(st.teams||{});
    setCourts(st.courts||[]);
    setCourtGroupPrimary(st.courtGroupPrimary||{});
    setGameDuration(st.gameDuration||30);
    setLinkedGroups(st.linkedGroups||[]);
    setGroupMatchups(st.groupMatchups||{});
    setSchedule(null); setScheduleWarnings([]); setScheduleDates([]);
    setTab("groups");
  };
  ...
  const handleRename = (id, name) => {
    const st = loadTournament(id);
    if (st) saveTournament(id, name, st);
  ...
  const handleDelete = (id) => {
    deleteTournament(id);
    if (id===tournamentId) {
      setTournamentId(null); setTournamentName("");
  ...
  const removeTeam=(gid,tid)=>{
    ...
    setGroupMatchups(m=>{const next={...m};for(const g2 in next){next[g2]=new Set([...next[g2]].filter(k=>!k.includes(tid)));}return next;});
  };
  ...
  const getEnabledSet=(gid,teamIds)=>{
    ...
    return new Set(roundRobinPairs(teamIds).map(([a,b])=>matchKey(a,b)));
  };
  ...
  const selectAllMatchups=(gid,teamIds)=>{
    setGroupMatchups(m=>({...m,[gid]:new Set(roundRobinPairs(teamIds).map(([a,b])=>matchKey(a,b)))}));
  };
  const clearAllMatchups=gid=>setGroupMatchups(m=>({...m,[gid]:new Set()}));
  ...
  const addCourt=()=>{
    ...
    const id="c"+uid();
    setCourts(c=>[...c,{id,name:newCourtName.trim(),location:newCourtLoc.trim(),windows:[{id:"w"+uid(),date:today,open:"08:00",close:"17:00"}]}]);
  ...
  const buildSchedule=()=>{
    ...
    for(const group of groups)effectiveMatchups[group.id]=getEnabledSet(group.id,group.teams);
    ...
    setSchedule(res.slots);setScheduleWarnings(res.warnings||[]);setScheduleDates(res.sortedDates||[]);
    setTab("schedule");
  };

  const allTeams=Object.values(teams);
  const totalEnabledMatches=useMemo(()=>groups.reduce((s,g)=>s+getEnabledSet(g.id,g.teams).size,0),[groups,groupMatchups]);
  const totalSlots=courts.reduce((s,c)=>s+(c.windows||[]).reduce((ws,w)=>{
    if(!w.date||!w.open||!w.close)return ws;const diff=timeMins(w.close)-timeMins(w.open);return ws+(diff>0?Math.floor(diff/gameDuration):0);
  },0),0);
  ...
  return (
    <div style={{minHeight:"100vh",background:P.bg,color:P.text,fontFamily:"'Inter','Segoe UI',sans-serif",fontSize:14}}>
      ...
      <div style={{background:P.surface,borderBottom:`1px solid ${P.border}`,padding:"13px 22px",display:"flex",alignItems:"center",gap:14}}>
        {/* Tournament picker button */}
        <button onClick={()=>setDrawerOpen(true)} style={{
          background:P.bg,border:`1px solid ${P.border}`,borderRadius:8,
          padding:"7px 12px",cursor:"pointer",color:P.text,fontFamily:"inherit",
          display:"flex",alignItems:"center",gap:8,transition:"all .15s",
        }}>
          <span style={{fontSize:16}}>📋</span>
          <div style={{textAlign:"left"}}>
            <div style={{fontWeight:700,fontSize:13,color:tournamentName?P.accent:P.muted}}>
        ...
        <input
          ...
          style={{background:"transparent",border:"none",borderBottom:`1px solid ${P.border}`,
            color:P.text,fontWeight:600,fontSize:15,fontFamily:"inherit",outline:"none",
            width:220,padding:"2px 4px"}}
        />

        <div style={{color:P.muted,fontSize:12,flex:1}}>
          {groups.length} group{groups.length!==1?"s":""} · {allTeams.length} teams · {totalEnabledMatches} matchups · {courts.length} courts
        </div>
        ...
        <Btn onClick={buildSchedule} disabled={allTeams.length<2||courts.length===0||totalEnabledMatches===0} style={{whiteSpace:"nowrap"}}>
          ⚡ Generate
        </Btn>
      </div>
      ...
      <div style={{padding:22,maxWidth:1000,margin:"0 auto"}}>
        ...
        {tab==="groups"&&(
          <div>
            ...
            {groups.length===0&&<Card style={{textAlign:"center",color:P.muted,padding:40}}>No groups yet. Add one above.</Card>}
                      ...
                      <div style={{borderTop:`1px solid ${P.border}`,paddingTop:14}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                          <div style={{display:"flex",alignItems:"center",gap:8}}>
                            ...
                            }}>{enabledCount} / {pairs.length} selected</span>
                          </div>
                          <div style={{display:"flex",gap:6}}>
                            <Btn variant="ghost" small onClick={()=>selectAllMatchups(group.id,group.teams)} disabled={allOn}>Select All</Btn>
                            <Btn variant="ghost" small onClick={()=>clearAllMatchups(group.id)} disabled={allOff}>Clear All</Btn>
                          </div>
                        </div>
                        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                          {pairs.map(([a,b])=>{
                            const key=matchKey(a,b);const enabled=enabledSet.has(key);
                            const teamA=teams[a];const teamB=teams[b];
                            if(!teamA||!teamB)return null;
                            return <MatchupChip key={key} teamA={teamA} teamB={teamB} enabled={enabled} onToggle={()=>toggleMatchup(group.id,group.teams,key)}/>;
                          })}
                        </div>
                        {allOff&&<div style={{marginTop:10,color:P.red,fontSize:12}}>⚠️ No matchups selected — this group won't appear in the schedule.</div>}
                      </div>
                    )}
                  </Card>
                );
                ...
                return (
                  <Card key={court.id}>
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
                      ...
                      <input value={court.location} onChange={e=>updCourt(court.id,"location",e.target.value)} placeholder="Location…"
                      ...
                      <div key={court.id} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"8px 0",borderTop:`1px solid ${P.border}`}}>
                        ...
                        <div style={{display:"flex",flexWrap:"wrap",gap:5,flex:1}}>
                          {dateList.length===0?<span style={{color:P.border,fontSize:12}}>No dates set</span>:dateList.map(d=>{
                            ...
                            return <span key={d} style={{background:P.surfaceLight,border:`1px solid ${P.border}`,borderRadius:6,padding:"3px 8px",fontSize:12}}>
                              <span style={{color:P.blue,fontWeight:600}}>{fmtDate(d)}</span>
                              <span style={{color:P.muted,marginLeft:5}}>{wins.map(w=>`${fmtTime(timeMins(w.open))}–${fmtTime(timeMins(w.close))}`).join(", ")}</span>
                            </span>;
                          })}
                        </div>
          ...
          <div>
            ...
            <Card style={{maxWidth:420}}>
              ...
              <div style={{background:P.bg,borderRadius:8,padding:14,border:`1px solid ${P.border}`}}>
                <Lbl>Summary</Lbl>
                {[["Selected matchups",totalEnabledMatches],["Courts",courts.length],["Available slots",totalSlots],["Capacity",totalSlots>=totalEnabledMatches?"✅ Enough":"⚠️ May not fit"]].map(([l,v])=>(
          ...
          <div>
            ...
            <Card style={{marginBottom:16}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:13}}>
                {allTeams.map(team=>{const sel=linkSelections.includes(team.id);return <div key={team.id} onClick={()=>toggleLinkSel(team.id)}
        ...
        {tab==="schedule"&&(
          <div>
            ...
            {schedule&&schedule.length===0&&scheduleWarnings.length===0&&<Card style={{textAlign:"center",padding:40,color:P.muted}}>No games placed. Add court windows with dates.</Card>}
